...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    const searchValue = common_vendor.ref("0");
    const input = (res2) => {
    };
    const blur = (res2) => {
    ...
    const onSearch = () => {
      ...
      searchData();
      common_vendor.index.__f__("log", "at pages/search/search.vue:113", queryParams.value.keyword);
    };
    ...
    const searchData = async () => {
      try {
        ...
        if (res.data.length == 0 && classList.value.length == 0)
          noSearch.value = true;
        common_vendor.index.__f__("log", "at pages/search/search.vue:151", res);
      } finally {
