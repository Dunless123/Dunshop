  ...
  <div class="service-management">
    <div class="card">
      <div class="flex justify-between items-center mb-20">
        <h3>发送通知</h3>
        <button class="btn btn-primary" @click="openSendModal">发送通知</button>
      </div>
    ...
    <div v-if="showSendModal" class="modal-overlay" @click="closeSendModal">
      <div class="modal-content" @click.stop>
        <div class="modal-header">
          <h3>发送通知</h3>
          <button class="close-btn" @click="closeSendModal">×</button>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label>用户ID</label>
            <input type="number" v-model="sendForm.userId" placeholder="请输入用户ID" />
          </div>
          <div class="form-group">
            <label>通知类型</label>
          ...
          <div class="form-group">
            <label>通知内容</label>
            <textarea v-model="sendForm.content" placeholder="请输入通知内容" rows="4"></textarea>
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-default" @click="closeSendModal">取消</button>
          <button class="btn btn-primary" @click="submitSend" :disabled="!sendForm.userId || !sendForm.content">发送</button>
        </div>
      </div>
    </div>
...
const fetchNotifications = async () => {
  try {
    ...
    notifications.value = [
      { id: 1, type: '订单状态变更', userId: 2, username: '张三', content: '您的订单已支付成功', status: '已送达', sendTime: '2026-04-24 10:00' },
      { id: 2, type: '系统通知', userId: 3, username: '李四', content: '您的认证信息已通过审核', status: '已送达', sendTime: '2026-04-24 09:30' },
      { id: 3, type: '订单状态变更', userId: 4, username: '王五', content: '您的订单已发货', status: '已送达', sendTime: '2026-04-24 09:00' },
      { id: 4, type: '订单状态变更', userId: 5, username: '赵六', content: '您的订单已完成', status: '已送达', sendTime: '2026-04-23 18:00' },
      { id: 5, type: '系统通知', userId: 6, username: '孙七', content: '您的认证信息审核失败', status: '已送达', sendTime: '2026-04-23 12:00' }
    ]
    total.value = 5
  }
}
...
const fetchTemplates = async () => {
  try {
    ...
    templates.value = [
      { id: 1, name: '订单支付成功', content: '您的订单{orderNo}已支付成功，感谢您的购买！', type: '订单状态变更' },
      { id: 2, name: '订单发货通知', content: '您的订单{orderNo}已发货，请注意查收！', type: '订单状态变更' },
      { id: 3, name: '认证通过通知', content: '您的认证信息已通过审核，现在可以使用更多功能！', type: '系统通知' },
      { id: 4, name: '认证失败通知', content: '您的认证信息审核失败，请检查后重新提交！', type: '系统通知' }
    ]
  }
}
...
const submitSend = async () => {
  if (!sendForm.value.userId || !sendForm.value.content) return
